...
const AddSemester: React.FC = () => {
  ...
  const addNextPeriod = () => {
    ...
    const end = formData.periods.length === periodNamesOrder.length - 1 ? formData.semester_end : "";

    setFormData(f => ({
      ...f,
      periods: [...f.periods, { name: nextName, start_date: start, end_date: end }]
    }));
  };
  ...
  return (
    <div className="space-y-4">
      ...
      <Breadcrumbs crumbs={[{ label: "Semesters", to: "/semester" }, { label: "Add Semester" }]} />
      <Stepper />
      ...
      <div className="bg-white rounded-lg shadow-lg p-6 space-y-6">
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-lg border border-gray-200 shadow-lg overflow-hidden">
              <div className="bg-blue-600 px-4 py-3 shadow">
                <h4 className="font-semibold text-white">Semester Information</h4>
              </div>
              <div className="p-4">
        ...
        {step === 2 && (
          <div className="space-y-4">
            <p className="text-sm text-gray-600">Add academic periods in order: Prelim, Midterm, Prefinal, Finals</p>
              ...
              {formData.periods.map((period, index) => (
                <div key={period.name} className="border border-gray-200 rounded-lg p-4 shadow-sm bg-gray-50">
                  <div className="flex justify-between items-center mb-4">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {index === formData.periods.length - 1 && (
                        <button type="button" onClick={removeLastPeriod} className="text-red-500 hover:text-red-700 transition" title="Remove this period">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <DateInput label="Start Date" value={period.start_date} min={formData.semester_start} max={formData.semester_end} onChange={(val) => updatePeriod(index, "start_date", val)} required />
                    <DateInput label="End Date" value={period.end_date} min={period.start_date || formData.semester_start} max={formData.semester_end} onChange={(val) => updatePeriod(index, "end_date", val)} required />
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="flex gap-2">
              {formData.periods.length > 0 && (
                <button type="button" onClick={removeLastPeriod} className="flex-1 py-2 px-4 rounded-md border border-red-200 text-red-600 hover:bg-red-50 transition shadow-sm font-medium">Remove Last</button>
              )}
              <button type="button" onClick={addNextPeriod} disabled={formData.periods.length === periodNamesOrder.length || (formData.periods.length > 0 && (!formData.periods.at(-1)?.start_date || !formData.periods.at(-1)?.end_date))} className={`flex-[2] py-2 px-4 rounded-md transition font-medium ${formData.periods.length === periodNamesOrder.length || (formData.periods.length > 0 && (!formData.periods.at(-1)?.start_date || !formData.periods.at(-1)?.end_date)) ? "bg-gray-400 text-gray-700 cursor-not-allowed" : "bg-blue-500 text-white hover:bg-blue-600 shadow-sm"}`}>
                {formData.periods.length === 0 ? "Add First Period" : "Add Next Period"}
              </button>
            </div>
        ...
        {step === 3 && (
          <div className="space-y-6">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div className="bg-white rounded-lg border border-gray-200 shadow-lg overflow-hidden">
                <div className="bg-blue-600 px-4 py-3 shadow"><h4 className="font-semibold text-white">Semester Information</h4></div>
              ...
              <div className="bg-white rounded-lg border border-gray-200 shadow-lg overflow-hidden">
                <div className="bg-blue-600 px-4 py-3 shadow"><h4 className="font-semibold text-white">Period Information</h4></div>
                <div className="p-4 space-y-3">
            ...
            <div className="flex justify-between pt-6 border-t border-gray-200">
              ...
              <button type="button" disabled={isPending} className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2 text-sm rounded-md shadow transition disabled:bg-blue-300 flex items-center gap-2" onClick={() => createSemester({ ...formData, status: "inactive" })}>
