...
const EditSchedule: React.FC = () => {
  ...
  const { data: courseData, isLoading: isFetching } = useQuery({
    ...
    queryFn: async () => {
        ...
        const response = await apiService.get<ApiResponse>(`/course/${id}/details`);
  ...
  useEffect(() => {
    if (courseData) {
        let derivedUnits = "";
        const lectureSched = courseData.schedules.find((s: any) => s.schedule_type === "Lecture");
        
        if (lectureSched && lectureSched.units !== undefined && lectureSched.units !== null) {
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-lg border border-gray-200 shadow-sm overflow-hidden">
              ...
              <div className="p-4 grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                  <label className="text-sm font-medium text-gray-700 block mb-1">Units (1-3) <span className="text-red-500">*</span></label>
            ...
            <div className="flex justify-end pt-4">
              <button onClick={() => validateStep1() && setStep(2)} className="bg-blue-600 text-white px-6 py-2 rounded-md shadow hover:bg-blue-700">Next</button>
            </div>
          </div>
        )}
